...
// ─── Segment photo with fallback ─────────────────────────────────────────────

function SegmentPhoto({
  src, alt, accentColor, icon: Icon, iconBgClass, tag, tagClass,
}: {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-graphite-100">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-graphite-400 hover:text-graphite-700 hover:bg-graphite-100 transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        {!done ? (
          <>
            ...
            <div className="flex items-center gap-2 px-6 pt-5">
              {steps.map((_, i) => (
                <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'bg-glass-600 w-8' : i < step ? 'bg-glass-300 w-4' : 'bg-graphite-200 w-4'}`} />
              ))}
              <span className="ml-auto text-xs text-graphite-400">{step + 1} / {steps.length}</span>
            </div>
              ...
              {currentStep.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl text-sm font-medium border transition-all duration-150 ${selected ? 'bg-glass-600 border-glass-600 text-white shadow-sm' : 'bg-white border-graphite-200 text-graphite-700 hover:border-glass-400 hover:text-glass-600'}`}
                  >
                    {opt.label}
                  </button>
                )
            ...
            <div className="px-6 pb-6 flex items-center justify-between border-t border-graphite-100 pt-4">
              ...
              <button
                onClick={handleNext}
                disabled={!canNext}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${canNext ? 'bg-glass-600 text-white hover:bg-glass-700 shadow-glass' : 'bg-graphite-100 text-graphite-400 cursor-not-allowed'}`}
              >
          ...
          <div className="px-6 py-8 flex flex-col items-center text-center gap-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              ...
              <button onClick={onClose} className="px-5 py-3 rounded-xl border border-graphite-200 text-graphite-600 text-sm font-medium hover:border-graphite-400 transition-colors">
                Cerrar
              </button>
            </div>
          </div>
...
export default function SegmentSelector() {
  ...
  const handleClick = (seg: Segment) => {
    if (seg.directLink) {
      const el = document.querySelector(seg.directLink.replace('/', ''))
          ...
          {segments.map((seg) => (
            ...
            >
              ...
              <SegmentPhoto
                ...
                icon={seg.icon}
                iconBgClass={seg.iconBgClass}
                tag={seg.tag}
                tagClass={seg.tagClass}
              />
              ...
              <div className="flex flex-col gap-3 p-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-graphite-900 text-base leading-tight">
                      {seg.title}
                    </h3>
                    <p className="text-xs font-semibold text-graphite-400 uppercase tracking-wide mt-0.5">
                      {seg.subtitle}
                    </p>
                  </div>
                </div>
